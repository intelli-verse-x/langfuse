    ...
    <div className="flex shrink-0">
      ...
      {treeLines.map((hasLine, index) => (
        <div key={index} className="relative w-3">
          {hasLine && (
            <div className="bg-border-contrast absolute top-0 bottom-0 left-1.5 w-px" />
          )}
        </div>
      ))}
      ...
      <div className="relative w-3">
        <div
          className={cn(
            "bg-border-contrast absolute top-0 left-1.5 w-px",
            isLastSibling ? "h-1/2" : "h-full",
          )}
        />
        <div className="bg-border-contrast absolute top-1/2 left-1.5 h-px w-1.5" />
      </div>
    </div>
  );
});
