...
const requiredSingleNodeRedisEnv = {
...
const setRequiredEnv = (
  redisEnv: Record<string, string> = requiredSingleNodeRedisEnv,
) => {
  clearRedisModeEnv();
...
describe("BullMQ Redis version check options", () => {
  ...
  test("skips Redis version checks in cluster mode", async () => {
    process.env.LANGFUSE_BULLMQ_SKIP_REDIS_VERSION_CHECK = "true";

    const { redisInstances, redisModule } = await importRedisModule({
      ...requiredSingleNodeRedisEnv,
      REDIS_CLUSTER_ENABLED: "true",
      REDIS_CLUSTER_NODES: "127.0.0.1:6370,127.0.0.1:6371",
    });
    const options = redisModule.createBullMQQueueOptionsWithRedis("test-queue");
